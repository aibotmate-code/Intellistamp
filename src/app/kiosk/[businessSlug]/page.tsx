...
export default function KioskPage({ params }: PageParams) {
  ...
  const handlePinSubmit = async () => {
    ...
    // Verify PIN against business - we do a quick pre-check by attempting a
    // dummy stamp with an invalid phone; the real verification happens server-side.
    // For UX, we locally validate by checking the pin format, then show the stamp area.
    // True server-side PIN verification happens on stamp submission.
    setKioskState('ready')
  ...
  const handleStamp = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 401) {
          // PIN was wrong after all — go back to PIN entry
          setPin('')
  ...
  // ── Shake animation for PIN button ──────────────────────────────────────────
  const shakePin = () => {
        ...
        <div className="w-full max-w-sm space-y-6">
            ...
            >
              <div className="text-center space-y-1">
                <div
                  ...
                  style={{ background: 'var(--color-elevated)', color: 'var(--color-green)' }}
                >
          ...
          {kioskState === 'stamping' && (
            <div className="text-center py-8 space-y-3">
              <div
                className="text-4xl animate-spin inline-block"
                style={{ color: 'var(--color-gold)' }}
              >
                ◌
              </div>
          ...
          {kioskState === 'success' && (
            <div
              className="rounded-2xl p-6 border text-center space-y-4 scale-in"
              style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}
            >
          ...
          {kioskState === 'cooldown' && (
            ...
            >
              ...
              <p className="font-bold" style={{ color: 'var(--color-text)' }}>
                Stamp cooldown active
              </p>
              <p className="text-sm" style={{ color: 'var(--color-text-muted)' }}>
                This customer already received a stamp recently.
                Next stamp available in {cooldownHours}h.
              </p>
              <Button onClick={resetForNext} variant="secondary" className="w-full">
                Next Customer →
              </Button>
            </div>
          )}
